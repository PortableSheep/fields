...
- Name: app.spec.ts >> App loading >> tool buttons exist but PDF-dependent ones are disabled
...
Locator: locator('button[title="Zoom In (+)"]')
Expected: disabled
Timeout: 5000ms
...
Call log:
  ...
  - waiting for locator('button[title="Zoom In (+)"]')
